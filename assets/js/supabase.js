...
import { EVALUATION_BASE } from './evaluation.js';
...
export async function signUp(email,password,nickname){localStorage.setItem(PENDING,String(nickname||'Игрок').trim().slice(0,24));const redirect=`${location.origin}${EVALUATION_BASE}`;const {data,error}=await supabase.auth.signUp({email,password,options:{emailRedirectTo:redirect}});if(error)throw error;if(data.session)await ensureProfile(nickname);return data}
